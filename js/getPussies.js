...
bitcoinInitializedPromise.then(() => {


  const NETWORK = bitcoin.networks.testnet;
  const MEMPOOLAPI_URL = 'https://mempool.space';
  
  async function generatePSBT(input, outputs) {
    const [inputTxId, inputIndex] = input.split('i');
    const tx = bitcoin.Transaction.fromHex(
      await fetch(`${MEMPOOLAPI_URL}/testnet/api/tx/${inputTxId}/hex`)
          .then(
              response => {
    ...
    outputs.forEach(output => {
      ...
      const value = parseInt(output[address]);
      psbt.addOutput({
        address: address,
        value: value,
      });
  ...
  const maker_input = 'cd9c706039c753744777603cd6a678ef6fc7cc3773a69f8b44c89c492d9bad14i0'; // Inscription
  const maker_outputs = [
    { 'tb1pkqtlr8rvhgmrkvmgfcmf24545dxzuxhy4dh46y2khxq2lfr3eu3smtzg2u': '6900000' }         // Maker address and ask value
  ];
  
  generatePSBT(maker_input, maker_outputs).then(psbtBase64 => {
    console.log(psbtBase64);
  }).catch(error => {
    console.error(error);
  });

  const taker_input = ''; // Payment input
  const taker_outputs = [
    { 'tb1pkqtlr8rvhgmrkvmgfcmf24545dxzuxhy4dh46y2khxq2lfr3eu3smtzg2u': '6900000' }         // Taker address and change value
  ];

  generateBuyerPSBT(taker_input, taker_outputs).then(psbtBase64 => {
    console.log(psbtBase64);
  }).catch(error => {
    console.error(error);
  });
